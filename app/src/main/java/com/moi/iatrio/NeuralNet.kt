 ...
 * Il apprend de nouvelles classes à la volée et se sauvegarde sur le téléphone.
 */
class NeuralNet(private val nIn: Int, private val nHid: Int) {
    ...
    private fun ensureLabel(label: String): Int {
        ...
        if (i < 0) {
            labels.add(label)
            w2.add(DoubleArray(nHid) { (Random.nextDouble() - 0.5) * 0.1 })
            b2.add(0.0)
            i = labels.size - 1
        }
        return i
    ...
    private fun forward(x: DoubleArray): Pair<DoubleArray, DoubleArray> {
        ...
        for (j in 0 until nHid) {
            ...
            h[j] = if (s > 0) s else 0.01 * s // leaky ReLU
        }
    ...
    /** Entraînement par rétropropagation, directement sur le téléphone. */
    ...
    fun load(f: File) {
        try {
            ...
            labels.clear(); w2.clear(); b2.clear()
            if (lines[0].isNotBlank()) labels.addAll(lines[0].split("|"))
            b1 = lines[1].split(",").map { it.toDouble() }.toDoubleArray()
            var idx = 2
            for (j in 0 until nHid) w1[j] = lines[idx++].split(",").map { it.toDouble() }.toDoubleArray()
            if (labels.isNotEmpty()) {
                b2.addAll(lines[idx++].split(",").map { it.toDouble() })
                repeat(labels.size) {
                    w2.add(lines[idx++].split(",").map { it.toDouble() }.toDoubleArray())
                }
            }
        } catch (e: Exception) { /* fichier corrompu : on repart de zéro */ }
    }
}
